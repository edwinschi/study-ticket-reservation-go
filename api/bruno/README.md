...
This folder contains a versioned Bruno collection for testing the Ticket Reservation Go Lab API.
...
This matches the API port exposed by `docker-compose.yml`.
...
Copy the sample file if you want to keep local-only values outside the versioned environment files:
...
The app currently uses an HTTP-only `visitor_session` cookie for authenticated reservation flows.
Run `sessions/Create Anonymous Session` first. Bruno should keep the returned cookie in its cookie
jar for the next requests.
